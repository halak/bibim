...
namespace Bibim
{
    ...
    float SequenceTemplate<T, a, b, c, d>::GetDuration()
    {
        ...
        {
            duration = 0.0f;
            for (std::vector<Keyframe<T> >::const_iterator it = keyframes.begin(); it != keyframes.end(); it++)
    ...
    {
        float time = 0.0f;
        for (std::vector<Keyframe<T> >::iterator it = keyframes.begin(); it != keyframes.end(); it++)
        {
            (*it).StartTime = time;
            time += (*it).Duration;
        }
